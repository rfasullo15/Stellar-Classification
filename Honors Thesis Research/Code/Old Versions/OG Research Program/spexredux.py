...
def extract(datadir,srcnames,srcfiles,biasfiles,flatfiles,alldarks,
    ...
    for i in loop_over:
        ...
        plt.figure()
        plt.plot(spx_redux[:,2,i])
        plt.xlabel('Pixel Value')
        plt.ylabel('(Raw - Sky) Intensity')
        plt.title(srcnames[i])
    ...
    spec_w = wcal.wavecal(spx_redux,wvvals)
    ...
    plt.figure()
    for i in loop_over:
        plt.plot(spx_redux[:,0,i],spx_redux[:,3,i]+(i*0.4))
        plt.text(4600,1.05+(i*0.4),srcnames[i])
    plt.xlabel(r'$\lambda$ ($\AA$)')
    plt.ylabel('Rectified Intensity + Offset')
    plt.title('All Spectra are Extracted!')
    plt.show(block=False)
    time.sleep(5)
    plt.close()
