...
class Spectrum:
    ...
    def __init__(self, df, name = None, classification = "-"):
        ...
        self.df = df              #6x2048 table of spectrum values. "wv", "rms", "raw", "snr", "rect", "wvcal". The column "local" will be added at a later time.
    ...
    def genSingleHydro(self, center, step, name = None):
        ...
        hydrodf = {"wv":  self.df["wv"][rightIndex:leftIndex].values, "rms": self.df["rms"][rightIndex:leftIndex].values, "raw": self.df["raw"][rightIndex:leftIndex].values, "snr": self.df["snr"][rightIndex:leftIndex].values, "rect": self.df["rect"][rightIndex:leftIndex].values, \
                        "wvcal": self.df["wvcal"][rightIndex:leftIndex].values, "local": self.df["local"][rightIndex:leftIndex].values}
