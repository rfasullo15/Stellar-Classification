...
class classification(tk.Frame):
    ...
    def createTrainingSet(self):
        ...
        data_amount = 30
        try:
            classtable_path = self.main_path + "\\Classtable" + self.spType+ ".txt"
            ...
            '''
            list_path = self.main_path + "\\Classifications.txt"
            ...
            gen_path = "C:\Users\90rfa\Documents\Honors Thesis Data\Generator2\\ "
            ...
            for name in pathtable["Name"]:
                ...
                for val in range(data_amount):
                    gen_name = str(count) + "G2" + str(val).zfill(2) + "-" + name
                    print(gen_name)
    ...
    def classify(self):
        classtable_path = self.main_path + "\\Classtable_ALL.txt"
        tree_path = self.main_path +"\\forest.sav"
        ...
        try:
            ...
            label_path = "C:\Users\90rfa\Dropbox\Honors Thesis Research\Code\GiraffeButts 2.3\Classifications_ALL.txt"
        ...
        self.createTrainingSet()
        class_mapping = {label:idx for idx, label in enumerate(np.unique(self.classtable["Classification"]))} #Testing set mapping (The mapping the tree uses)

        #print(other_mapping)
        #curr_mapping = {label:idx for idx, label in enumerate(np.unique(test_classtable["Classification"]))} #Real world set mapping 


        collect = []
        
        for key, value in class_mapping.iteritems():
            frame = test_classtable.loc[test_classtable["Classification"] == key]
        ...
        master_table = pd.DataFrame(data = masterlist, columns = self.headers)

        #master_table["Classification"] = master_table["Classification"].map(class_mapping)

        print(master_table)

        #print(master_table)

        #newest_mapping = {label:idx for idx, label in enumerate(np.unique(master_table["Classification"]))      
        ...
        #maps = pd.DataFrame(class_mapping)
        #print("TEST")
        #print(class_mapping)
        ...
        answers = []
        data = []
        for spect in master_table.values:
            answers.append(str(spect[1]))
            data.append(spect[2:].tolist())

        predictions = classifier.predict(data)
        scores = classifier.score(data, answers)
